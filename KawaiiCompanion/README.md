...
| `/kc info` | show her status (name, heal aura, form) |
...
While she's summoned, your companion periodically **tops up your HP** when you're hurt and standing nearby, with heart particles and a soft chime (config `heal-aura:`). Tune the pulse interval, hearts per pulse, and range — or turn it off entirely. `/companion info` shows whether the aura is on.

She also re-targets mobs that hit you (the `combat-smarts.assist-owner` setting), and the real-entity companion can never itself be targeted by mobs.
